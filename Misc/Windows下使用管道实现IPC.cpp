...
// 观察到Linux中的管道使用看起来是比Windows中使用起来更简单,需要的参数也更少
// 进程使用管道进行通信时,实际上是在调用特定的系统调用 system call 来获取操作系统提供的支持
// 系统调用是用户进程与操作系统内核之间交互的一种方式,允许进程请求内核执行某些特定的操作

// 下面两行处于Linux下
// 匿名管道 不涉及文件系统,也不会进行磁盘操作.它们完全在内存中操作,是进程间通信的一种快速方法
// 命名管道 FIFO 需要使用文件系统,因为它们是在文件系统中创建的特殊文件.然而,实际的数据传输仍然在内存中进行,不涉及 磁盘I/O 操作
// Windows:
// Windows 的命名管道类似于 Linux 的命名管道,但不需要在文件系统中创建一个实际的文件.管道名称仅作为 标识符 使用.数据传输同样在 内存 中完成.
int main()
{
	// HANDLE -> void *
